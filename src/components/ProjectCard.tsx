...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  if (layout === "grid") {
    return (
      <div
        className={`group bg-gray-800/50 rounded-2xl p-6 border border-gray-700 hover:border-green-500/50 transition-all duration-300 hover:transform hover:scale-105 ${className}`}
      >
        {/* Project Image */}
        <div
          className={`relative overflow-hidden rounded-xl mb-6 ${bgColor} p-4 aspect-video`}
        >
          <img
            src={image}
            alt={title}
        ...
        <div className="space-y-4">
          <div>
            <div className="inline-block px-3 py-1 bg-green-900/30 rounded-full mb-3">
    ...
    return (
      <div className={`project ${className}`}>
        ...
        <div className="text-content space-y-4 p-4">
          ...
          <div className="mt-4">
            <ModernButton href={`/projects/${id}`} variant="primary" size="sm">
              View Project →
            </ModernButton>
          </div>
        </div>
  ...
  return (
    <div className={`group ${className}`}>
      ...
      <div className="space-y-4">
        <div>
          <div className="text-sm text-green-400 mb-2">{category}</div>
